...
World::init()
{
  ...
  // eye_location = pCoor(17.9,-14,117.2); //for eye location changed
  eye_location = pCoor(0,-14,250.2);
  //  eye_direction = pVect(-0.15,-0.06,-0.96);
  eye_direction = pVect(-0.0,-0.06,-0.96);

  opt_platform_curved = false;
  platform_box = NULL;
  platform_xmin = -100; platform_xmax = 100;
...
World::time_step_cpu()
{
  ...
  /// This is the scene setup simulation for flatly arranged Carbon Nanotube.
  // this loop identifies Neighboring Carbon Nanotube
...
if(Clist.occ()>9)
	 ...
	 for (int k=1;k<=4;k++)
	   {
	     for (int j = 0;j<len_of_nanotube;j++ )
	       {
		 if(k==1 && i==1) continue;
		 int temp1= (((len_of_nanotube*(k-1))+(j))+((len_of_nanotube*4*(i-1))));
		 if(k==1)
		   {
		     Clist[temp1]->ballpos1=Clist[temp1-(len_of_nanotube)]->position;
		     Clist[temp1]->bpos1bond=true;
		      Clist2[temp1]->ballpos1=Clist2[temp1-(len_of_nanotube)]->position;
		     Clist2[temp1]->bpos1bond=true;
		   }
		 ...
		 else if(k==2)
		   {
		     if(j!=0)
		       {
			 Clist[temp1]->ballpos1=Clist[temp1-(len_of_nanotube+1)]->position;
			 Clist[temp1]->bpos1bond=true;
			 Clist2[temp1]->ballpos1=Clist2[temp1-(len_of_nanotube+1)]->position;
			 Clist2[temp1]->bpos1bond=true;
		       }
		     Clist[temp1]->ballpos2=Clist[temp1-(len_of_nanotube)]->position;
		     Clist[temp1]->bpos2bond=true;
		     Clist2[temp1]->ballpos2=Clist2[temp1-(len_of_nanotube)]->position;
		     Clist2[temp1]->bpos2bond=true;
		   }
		 else if(k==3)
		   {
		     Clist[temp1]->ballpos1=Clist[temp1-len_of_nanotube]->position;
		 ...
		 else if(k==4)
		   {
		     ...
		     Clist2[temp1]->ballpos1=Clist2[temp1-(len_of_nanotube)]->position;
		     Clist2[temp1]->bpos1bond=true;
		     if(j!=(len_of_nanotube-1))
		       {
			 Clist[temp1]->ballpos2=Clist[temp1-(len_of_nanotube-1)]->position;
			 Clist[temp1]->bpos2bond=true;  
			 Clist2[temp1]->ballpos2=Clist2[temp1-(len_of_nanotube-1)]->position;
			 Clist2[temp1]->bpos2bond=true;  
		       }			   
		   }
		 else break;
  ...
  /// Apply gravitational force (except for scene_setup 1 & 2).
  //
 if(scene_change==3 || scene_change==4)
   {
   for ( Phys *p; phys_live_iterate(p); ) 
      {

	  p->velocity += gravity_accel_dt;
      }

   }
 
  /// This is the scene setup simulation for rolled arranged Carbon Nanotube.
  // this loop identifies Neighboring Carbon Nanotubes.

if(scene_change==2 || scene_change==3  || scene_change==4)
       {
  if(Clist.occ()>9)
   {
	     ...
	     for (int j = 0;j<len_of_nanotube;j++ )
	       {
		      ...
		      else if(k==2)
			{
			  if(j!=0)
			    ...
			    {
			      Clist[temp1]->ballpos1=Clist[temp1-(1)]->position;
			      Clist[temp1]->bpos1bond=true;
			    }
			 Clist[temp1]->ballpos2=Clist[temp1-(len_of_nanotube)]->position;
			 Clist[temp1]->bpos2bond=true;
		       ...
		       else if(k==4)
			 {
			   ...
			   if(j!=(len_of_nanotube-1))
			     ...
			     {
			       Clist[temp1]->ballpos2=Clist[temp1-((2*len_of_nanotube)-1)]->position;
			       Clist[temp1]->bpos2bond=true;
			     }		   
		         }
		       else break;
...
/// This is the scene setup simulation for rolled arranged Carbon Nanotube.
  // this loop identifies Neighboring Carbon Nanotubes and applies bonding force.
	     ...
	     for (int j = 0;j<len_of_nanotube;j++ )
	       {
		      ...
		      if(k==1)
			{
			  ...
			  if(Clist[temp1]->bpos1bond==true)
			    {
			      pNorm ball_to_neighbor1(Clist[temp1]->position,Clist[temp1-(len_of_nanotube)]->position);
			      pVect delta_v1 = Clist[temp1-(len_of_nanotube)]->velocity - Clist[temp1]->velocity;
			      float delta_s1 = dot( delta_v1, ball_to_neighbor1 );
			      const float spring_stretch1 = ball_to_neighbor1.magnitude - dist_bwt_atoms;
			      const bool gaining_e1 = ( delta_s1 > 0.0 ) == ( spring_stretch1 > 0 );
			      const float spring_constant1 = gaining_e1 ? opt_spring_constant : opt_spring_constant * 0.7;

			      force1 +=  spring_constant1 * spring_stretch1 * ball_to_neighbor1;
			      Clist[temp1]->velocity += ( force1 / Clist[1]->mass ) * delta_t;
			      Clist[temp1-(len_of_nanotube)]->velocity += (- force1 / Clist[1]->mass ) * delta_t;
			      const double fs1 = pow(1+opt_air_resistance,-delta_t);
			      Clist[temp1]->velocity *= fs1;
			      Clist[temp1-(len_of_nanotube)]->velocity *= fs1;
			    }
			}
		      ...
		      else if(k==2)
			{
			  ...
			  if(Clist[temp1]->bpos1bond==true)
			    {
			      ...
			      pNorm ball_to_neighbor2(Clist[temp1]->position,Clist[temp1-(len_of_nanotube+1)]->position);
			      pVect delta_v2 = Clist[temp1-(len_of_nanotube+1)]->velocity - Clist[temp1]->velocity;
			      float delta_s2 = dot( delta_v2, ball_to_neighbor2 );
			      const float spring_stretch2 = ball_to_neighbor2.magnitude - dist_bwt_atoms;
			      const bool gaining_e2 = ( delta_s2 > 0.0 ) == ( spring_stretch2 > 0 );
			      const float spring_constant2 = gaining_e2 ? opt_spring_constant : opt_spring_constant * 0.7;
			      			      
			      force2 +=  spring_constant2 * spring_stretch2 * ball_to_neighbor2;
			      Clist[temp1]->velocity += ( force2 / Clist[1]->mass ) * delta_t;
			      Clist[temp1-(len_of_nanotube+1)]->velocity += ( -force2 / Clist[1]->mass ) * delta_t;
			      const double fs2 = pow(1+opt_air_resistance,-delta_t);
			      Clist[temp1]->velocity *= fs2;
			      Clist[temp1-(len_of_nanotube+1)]->velocity *= fs2;
			   ...
			   else
			     {
			      ...
			      pNorm ball_to_neighbor2(Clist[temp1]->position,Clist[temp1-(1)]->position);
			      pVect delta_v2 = Clist[temp1-(1)]->velocity - Clist[temp1]->velocity;
			      float delta_s2 = dot( delta_v2, ball_to_neighbor2 );
			      const float spring_stretch2 = ball_to_neighbor2.magnitude - dist_bwt_atoms;
			      const bool gaining_e2 = ( delta_s2 > 0.0 ) == ( spring_stretch2 > 0 );
			      const float spring_constant2 = gaining_e2 ? opt_spring_constant : opt_spring_constant * 0.7;
			      ...
			      force2 +=  spring_constant2 * spring_stretch2 * ball_to_neighbor2;
			      Clist[temp1]->velocity += ( force2 / Clist[1]->mass ) * delta_t;
			      Clist[temp1-(1)]->velocity += ( -force2 / Clist[1]->mass ) * delta_t;
			      const double fs2 = pow(1+opt_air_resistance,-delta_t);
			      Clist[temp1]->velocity *= fs2;
			      Clist[temp1-(1)]->velocity *= fs2;
			     }

			 if(Clist[temp1]->bpos2bond==true)
			    {
			      pVect force3(0,0,0);
			      pNorm ball_to_neighbor3(Clist[temp1]->position,Clist[temp1-(len_of_nanotube)]->position);
			      pVect delta_v3 = Clist[temp1-(len_of_nanotube)]->velocity - Clist[temp1]->velocity;
			      float delta_s3 = dot( delta_v3, ball_to_neighbor3 );
			      const float spring_stretch3 = ball_to_neighbor3.magnitude - dist_bwt_atoms;
			      const bool gaining_e3 = ( delta_s3 > 0.0 ) == ( spring_stretch3 > 0 );
			      const float spring_constant3= gaining_e3 ? opt_spring_constant : opt_spring_constant * 0.7;

			      force3 +=  spring_constant3 * spring_stretch3 * ball_to_neighbor3;
			      Clist[temp1]->velocity += ( force3 / Clist[1]->mass ) * delta_t;
			      Clist[temp1-(len_of_nanotube)]->velocity += (- force3 / Clist[1]->mass ) * delta_t;
			      const double fs3 = pow(1+opt_air_resistance,-delta_t);
			      Clist[temp1]->velocity *= fs3;
			      Clist[temp1-(len_of_nanotube)]->velocity *= fs3;
			    }
		      ...
		      else if(k==3)
			 {
			   if(Clist[temp1]->bpos1bond==true)
			    {
			      pVect force4(0,0,0);
			      pNorm ball_to_neighbor4(Clist[temp1]->position,Clist[temp1-(len_of_nanotube)]->position);
			      pVect delta_v4 = Clist[temp1-(len_of_nanotube)]->velocity - Clist[temp1]->velocity;
			      float delta_s4 = dot( delta_v4, ball_to_neighbor4 );
			      const float spring_stretch4 = ball_to_neighbor4.magnitude - dist_bwt_atoms;
			      const bool gaining_e4 = ( delta_s4 > 0.0 ) == ( spring_stretch4 > 0 );
			      const float spring_constant4 = gaining_e4 ? opt_spring_constant : opt_spring_constant * 0.7;

			      force4 +=  spring_constant4 * spring_stretch4 * ball_to_neighbor4;
			      Clist[temp1]->velocity += ( force4 / Clist[1]->mass ) * delta_t;
			      Clist[temp1-(len_of_nanotube)]->velocity += (- force4 / Clist[1]->mass ) * delta_t;
			      const double fs4 = pow(1+opt_air_resistance,-delta_t);
			      Clist[temp1]->velocity *= fs4;
			      Clist[temp1-(len_of_nanotube)]->velocity *= fs4;
			    }
			 }
		       ...
		       else if(k==4)
			 {
			    ...
			    {
			      pVect force5(0,0,0);
			      pNorm ball_to_neighbor5(Clist[temp1]->position,Clist[temp1-(len_of_nanotube)]->position);
			      pVect delta_v5 = Clist[temp1-(len_of_nanotube)]->velocity - Clist[temp1]->velocity;
			      float delta_s5 = dot( delta_v5, ball_to_neighbor5 );
			      const float spring_stretch5 = ball_to_neighbor5.magnitude - dist_bwt_atoms;
			      const bool gaining_e5 = ( delta_s5 > 0.0 ) == ( spring_stretch5 > 0 );
			      const float spring_constant5 = gaining_e5 ? opt_spring_constant : opt_spring_constant * 0.7;

			      force5 += spring_constant5 * spring_stretch5 * ball_to_neighbor5;
			      Clist[temp1]->velocity += ( force5 / Clist[1]->mass ) * delta_t;
			      Clist[temp1-(len_of_nanotube)]->velocity += (- force5 / Clist[1]->mass ) * delta_t;
			      const double fs5 = pow(1+opt_air_resistance,-delta_t);
			      Clist[temp1]->velocity *= fs5;
			      Clist[temp1-(len_of_nanotube)]->velocity *= fs5;
			    ...
			    {
			      pVect force6(0,0,0);
			      pNorm ball_to_neighbor6(Clist[temp1]->position,Clist[temp1-(len_of_nanotube-1)]->position);
			      pVect delta_v6 = Clist[temp1-(len_of_nanotube-1)]->velocity - Clist[temp1]->velocity;
			      float delta_s6 = dot( delta_v6, ball_to_neighbor6 );
			      const float spring_stretch6 = ball_to_neighbor6.magnitude - dist_bwt_atoms;
			      const bool gaining_e6 = ( delta_s6> 0.0 ) == ( spring_stretch6 > 0 );
			      const float spring_constant6 = gaining_e6 ? opt_spring_constant : opt_spring_constant * 0.7;

			      force6 +=  spring_constant6 * spring_stretch6 * ball_to_neighbor6;
			      Clist[temp1]->velocity += ( force6 / Clist[1]->mass ) * delta_t;
			      Clist[temp1-(len_of_nanotube-1)]->velocity += (- force6 / Clist[1]->mass ) * delta_t;
			      const double fs6 = pow(1+opt_air_resistance,-delta_t);
			      Clist[temp1]->velocity *= fs6;
			      Clist[temp1-(len_of_nanotube-1)]->velocity *= fs6;
			    }
			     }	
			     else
			       {
				  pVect force6(0,0,0);
				  pNorm ball_to_neighbor6(Clist[temp1]->position,Clist[temp1-((2*len_of_nanotube)-1)]->position);
			      pVect delta_v6 = Clist[temp1-((2*len_of_nanotube)-1)]->velocity - Clist[temp1]->velocity;
			      float delta_s6 = dot( delta_v6, ball_to_neighbor6 );
			      const float spring_stretch6 = ball_to_neighbor6.magnitude - dist_bwt_atoms;
			      const bool gaining_e6 = ( delta_s6> 0.0 ) == ( spring_stretch6 > 0 );
			      const float spring_constant6 = gaining_e6 ? opt_spring_constant : opt_spring_constant * 0.7;
			      ...
			      force6 +=  spring_constant6 * spring_stretch6 * ball_to_neighbor6;
			      Clist[temp1]->velocity += ( force6 / Clist[1]->mass ) * delta_t;
			      Clist[temp1-((2*len_of_nanotube)-1)]->velocity += (- force6 / Clist[1]->mass ) * delta_t;
			      const double fs6 = pow(1+opt_air_resistance,-delta_t);
			      Clist[temp1]->velocity *= fs6;
			      Clist[temp1-((2*len_of_nanotube)-1)]->velocity *= fs6;
			       }
		         }
		       else break;
...
// this loop identifies Neighboring Carbon Nanotubes and checks if they are 120 degrees apart.

  // based on angle it applies force to neighboring carbon nanotubes.
 if(scene_change==2 || scene_change==3  || scene_change==4)
   {
	 ...
	 for (int k=1;k<=4;k++)
	   {
	     for (int j = 0;j<len_of_nanotube;j++ )
	       {
		      if(k==1 && i==1) continue;
		      if(k==4 && i==breadth_of_nanotube) continue;
		      int temp1= (((len_of_nanotube*(k-1))+(j))+((len_of_nanotube*4*(i-1))));
		      float angbb=((2*M_PI)/3);
		      
		      if(k==1)
			{
			  tempa1=(temp1-(len_of_nanotube));
			  tempa2=(temp1+(len_of_nanotube+1));
			  tempa3=(temp1+(len_of_nanotube));
			}
		      ...
		      else if(k==2)
			{
			  if(j!=0)
			    {
			       tempa1=(temp1-(len_of_nanotube+1));
			       tempa2=(temp1-(len_of_nanotube));
			       tempa3=(temp1+(len_of_nanotube));
			    }
			  else
			    {
		      ...
		      else if(k==3)
			 {
			   tempa1=(temp1-(len_of_nanotube));
			  tempa2=(temp1+(len_of_nanotube));
			  tempa3=(temp1+(len_of_nanotube-1));
			 }
		       ...
		       else if(k==4)
			 {
			   ...
			   if(j!=(len_of_nanotube-1))
			     ...
			     {
			       tempa1=(temp1-(len_of_nanotube));
			       tempa2=(temp1-((2*len_of_nanotube)-1));
			       tempa3=(temp1+(len_of_nanotube));
			     }		   
		         }
		       else break;
		      ...
		      float a1223= pangle(Clist[tempa1]->position,Clist[temp1]->position,Clist[tempa2]->position);
		      float b1223= pangle(Clist[tempa2]->position,Clist[temp1]->position,Clist[tempa3]->position);
		      float c1223= pangle(Clist[tempa3]->position,Clist[temp1]->position,Clist[tempa1]->position);
		      
		      // pCoor rand1=((Clist[tempa2]->position+Clist[temp1]->position)/2);
		      // pCoor rand2=((Clist[tempa3]->position+Clist[temp1]->position)/2);
		      // pCoor rand3=((Clist[tempa1]->position+Clist[temp1]->position)/2);
  
		      pVect force_a(0,0,0);
		      pNorm ball_to_neighbor_a(Clist[tempa1]->position,Clist[tempa2]->position);

		      pVect delta_va = Clist[tempa2]->velocity - Clist[tempa1]->velocity;
		      float delta_sa = dot( delta_va, ball_to_neighbor_a );
		      const float spring_stretcha = a1223 - angbb;
		      const bool gaining_ea = ( delta_sa> 0.0 ) == ( spring_stretcha > 0 );
		      const float spring_constanta = gaining_ea ? opt_spring_constant : opt_spring_constant * 0.5;

		      force_a +=  spring_constanta * spring_stretcha * ball_to_neighbor_a;
		      Clist[tempa1]->velocity += ( force_a / Clist[1]->mass ) * delta_t;
		      Clist[tempa2]->velocity += (- force_a / Clist[1]->mass ) * delta_t;
		      //		      const double fsa = pow(1+opt_air_resistance,-delta_t);
		      // Clist[tempa1]->velocity *= fsa;
		      // Clist[tempa2]->velocity *= fsa;

		      
     
		      pVect force_b(0,0,0);
		      pNorm ball_to_neighbor_b(Clist[tempa2]->position,Clist[tempa3]->position);

		      pVect delta_vb = Clist[tempa3]->velocity - Clist[tempa2]->velocity;
		      float delta_sb = dot( delta_vb, ball_to_neighbor_b );
		      const float spring_stretchb = b1223 - angbb;
		      const bool gaining_eb = ( delta_sb> 0.0 ) == ( spring_stretchb > 0 );
		      const float spring_constantb = gaining_eb ? opt_spring_constant : opt_spring_constant * 0.5;

		      force_b +=  spring_constantb * spring_stretchb * ball_to_neighbor_b;
		      Clist[tempa2]->velocity += ( force_b / Clist[1]->mass ) * delta_t;
		      Clist[tempa3]->velocity += (- force_b / Clist[1]->mass ) * delta_t;
		      // const double fsb = pow(1+opt_air_resistance,-delta_t);
		      //Clist[tempa2]->velocity *= fsb;
		      //Clist[tempa3]->velocity *= fsb;

     
		      pVect force_c(0,0,0);
		      pNorm ball_to_neighbor_c(Clist[tempa3]->position,Clist[tempa1]->position);

		      pVect delta_vc = Clist[tempa1]->velocity - Clist[tempa3]->velocity;
		      float delta_sc = dot( delta_vc, ball_to_neighbor_c );
		      const float spring_stretchc = c1223 - angbb;
		      const bool gaining_ec = ( delta_sc> 0.0 ) == ( spring_stretchc > 0 );
		      const float spring_constantc = gaining_ec ? opt_spring_constant : opt_spring_constant * 0.5;

		      force_c +=  spring_constantc * spring_stretchc * ball_to_neighbor_c;
		      Clist[tempa3]->velocity += ( force_c / Clist[1]->mass ) * delta_t;
		      Clist[tempa1]->velocity += (- force_c / Clist[1]->mass ) * delta_t;
		      // const double fsc = pow(1+opt_air_resistance,-delta_t);
		      //Clist[tempa3]->velocity *= fsc;
		      //Clist[tempa1]->velocity *= fsc;
...
/// This is the scene setup simulation for rolled arranged Carbon Nanotube.
  // this loop identifies Neighboring Carbon Nanotubes and applies compensated force 
 //for the other Degrees of Freedom in a perpendicular plane.
 ...
 for(int i = 1;i<= breadth_of_nanotube;i++)
	 ...
	 for (int k=1;k<=4;k++)
	   {
	     for (int j = 0;j<len_of_nanotube/2;j++ )
	       {
		 // if(k==1 && i==1) continue;
		 int temp1= (((len_of_nanotube*(k-1))+(j))+((len_of_nanotube*4*(i-1))));
		 int temp2= temp1 +(len_of_nanotube/2);
		 float dia_of_nano=(( ( 2*dist_bwt_atoms*cosf((2*M_PI)/6))*len_of_nanotube)/M_PI);
		     
		     
		      
		 pVect force_a1(0,0,0);
		 pNorm ball_to_neighbor_a1(Clist[temp1]->position,Clist[temp2]->position);

		 pVect delta_va1 = Clist[temp2]->velocity - Clist[temp1]->velocity;
		 float delta_sa1 = dot( delta_va1, ball_to_neighbor_a1 );
		 const float spring_stretcha1 = ball_to_neighbor_a1.magnitude -2* dia_of_nano;
		 const bool gaining_ea1 = ( delta_sa1> 0.0 ) == ( spring_stretcha1 > 0 );
		 const float spring_constanta1 = gaining_ea1 ? opt_spring_constant : opt_spring_constant * 0.5;

		 force_a1 +=  spring_constanta1 * spring_stretcha1 * ball_to_neighbor_a1;
		 Clist[temp1]->velocity += ( force_a1 / Clist[1]->mass ) * delta_t;
		 Clist[temp2]->velocity += (- force_a1 / Clist[1]->mass ) * delta_t;
...
World::cb_keyboard()
{
  ...
  case '1': setup_diff_nanotube(); break;
  case '2': setup_multinano(); break;
  case '3': setup_nanotube(5,6,6); break;
    // (dist,len,breadth)
  case '4': setup_nanotube_2(5,8,4); break;
